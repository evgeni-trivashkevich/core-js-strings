...
function removeFirstOccurrences(str, value) {
  ...
  return indexValue > -1
    ? str.substring(0, indexValue) + str.substring(indexValue + value.length)
    : str;
}
...
function formatTime(minutes, seconds) {
  const minutesSrt = minutes.toString();
  const secondsStr = seconds.toString();

  return `${minutesSrt.padStart(2, 0)}:${secondsStr.padStart(2, 0)}`;
}
...
function orderAlphabetically(str) {
  ...
  return strArr.sort((a, b) => collator.compare(a, b)).join('');
}
...
function countVowels(str) {
  const m = str.match(/[aeiouAEIOUY]/gi);
...
function isPalindrome(str) {
  ...
  return strNew === strNew.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  const arrSentence = sentence.split(' ').sort((a, b) => b.length - a.length);
...
function reverseWords(str) {
  ...
  const arrStrShench = arrStr.map((word) => word.split('').reverse().join(''));
  return arrStrShench.join(' ');
}
...
function invertCase(str) {
  const arrStr = str.split('');
  const arrStrShench = arrStr.map((word) =>
